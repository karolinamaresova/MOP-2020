...
if (isset($submit)) {
    ...
    $idCar = filter_input(INPUT_POST, 'type');
...
?>     </select><br>
          ...
          <?php   } elseif (in_array($_SESSION["userRole"], array(1))) {
            // admin sekce
            $allUsers = Model::getAllDrivers();
            $allCars = Model::getAllCars();
            $allRides = Model::getAllRides();
            ...
            <div class="row">
              <div class="input-field col s12">
                ...
                foreach ($allUsers as $user) {
                  if ($user['id_user'] == $ride['id_user']) {
                    ?>
                    <option value="<?= $user['id_user'] ?>" selected><?= $user['firstname'] . ' ' . $user['surname'] ?></option>
                    <?php
                    continue;
                  }
                  ?>
  ...
  foreach ($allCars as $car) {
      ...
      if ($car['id_car'] == $ride['id_car']) {
          ?> <option value="<?= $car['id_car'] ?> " selected><?= $car['type'] . ' | ' . $car['SPZ'] ?></option>
                  <?php continue;
      } ?>
